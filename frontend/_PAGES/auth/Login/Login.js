...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 403 && data.necesitaVerificar) {
          setNeedsVerification(true)
          setError('')
          return
        }
        setError(data.error || t('auth.login.errors.loginFailed'))
        return
      }
  ...
  async function verifyCode(e) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || t('auth.login.errors.invalidCode'))
        return
      }
  ...
  async function resendCode() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || t('auth.login.errors.resendFailed'))
        return
      }

      setError('')
      alert(t('auth.login.codeSent'))
    } catch (err) {
      setError(t('auth.login.errors.connection'))
    } finally {
      setLoading(false)
    ...
    return (
      <div className={styles.page}>
        ...
        <p className={styles.greeting}>
          {t('auth.login.verificationSubtitle')} <strong>{email}</strong>
        </p>
          ...
          <form className={styles.form} onSubmit={verifyCode}>
            <div className={styles.field}>
              <label className={styles.label}>{t('auth.login.verificationCode')}</label>
            ...
            <button type="submit" className={styles.submitBtn} disabled={loading || code.length !== 6}>
              {loading ? t('auth.login.verifying') : t('auth.login.verifyAccount')}
            </button>
          </form>
      ...
      <div className={styles.modal}>
        ...
        <div className={styles.footer}>
          <p className={styles.footerText}>
            {t('auth.login.noAccount')} <Link href="/signup/" className={styles.footerLink}>{t('auth.login.createAccount')}</Link>
          </p>
          <p className={styles.footerText}>
            {t('auth.login.forgotPassword')} <Link href="/recover/" className={styles.footerLink}>{t('auth.login.recover')}</Link>
          </p>
        </div>
      </div>
    </div>
